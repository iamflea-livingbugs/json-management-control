...
- **显示列配置** — 章节列表选择可见字段，树形视图展示字段值结构（对象/数组可展开查看子键），未配置时自动显示实际字段
...
│   │   ├── ColumnFieldNode.vue ← 显示列配置字段树（递归，顶层勾选/子键展开）
...
- [x] **显示列功能完善**：响应式修复（保存即时刷新）、对话框 Vue 化（`<Modal>` + 别名 + 全选/清空 + 草稿）、三态语义（未配置自动显示实际字段 / 显式选择 / 显式清空）
- [x] **列表表头**：列标题行（数组=说话人、对象=属性），i18n 字段附语言标签；speaker 列纳入配置可勾选/清空（含头像 Badge 联动）
- [x] **别名角标**：设置过字段标签的字段显示 🔖（章节列表头 + 表单），未设置不提供默认标签
- [x] **消除硬编码 i18n 判断**：新增 `getI18nMarker()`（读结构类型 marker），4 处写死 `'zh'` 判断统一读配置；新增 `hasFieldLabel()`
- [x] **显示列设置树形视图**（新增 `ColumnFieldNode.vue`）：递归字段树，顶层勾选作为列、对象/数组展开查看结构（零语义判断）；修复 v-model 绑 prop 导致的白屏编译错误
- [x] **cursor 修复**：全局 `input { cursor: text }` 误伤勾选框，恢复手型
