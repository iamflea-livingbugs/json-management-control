...
### v0.02 — 结构类型同步 + 布局优化
- [x] 结构类型增删改自动同步到当前章节数据（新增/删除字段、删除整个类型均同步）
- [x] 删除结构类型时保留 struct 标记键（如 zh），不误删匹配条件
- [x] 侧面板折叠/展开添加 width + opacity 平滑过渡动画（0.3s / 0.15s）
- [x] splitter 拖拽时禁用过渡，避免拖拽卡顿
- [x] 折叠时保存面板宽度（dataset.savedWidth），展开时恢复
- [x] 设置面板新增"恢复默认布局"按钮（左右面板宽度复位，不切换视图）
- [x] 保存宽度从闭包变量改为 DOM 属性，跨模块可读写
- [x] 文档更新：README.md + PROJECT_HANDOVER.md 新增"未来待实现的功能"
