...
import { loadTemplates, saveTemplate, getContextKeys, getContextsConfig, getFieldLabel, saveLabel } from './storyTypes.js';
    ...
    modal.innerHTML = `
        <div class="modal-box">
            ...
            <div class="modal-footer">
                <button class="btn btn-sm" id="btn-modal-close-bottom">关闭</button>
            </div>
        </div>`;
...
function renderModalContent() {
    ...
    const ctxBtns = ctxKeys.map(k => {
        ...
        return `<button class="tpl-ctx-btn ${k === _currentCtx ? 'active' : ''}" data-ctx="${k}" title="${cfg.description || ''}">${cfg.label || k}</button>`;
    }).join('');
    ...
    $('#btn-add-tpl-field').addEventListener('click', () => {
        ...
        let key = 'new_field';
        let i = 1;
        while (key in ctx) { key = 'new_field_' + i; i++; }
        const typeSelect = $('#tpl-add-type');
    ...
    if (editor) {
        editor.addEventListener('input', () => {
            applyHighlight(hlCode, editor.value);
        });
        ...
        editor.addEventListener('blur', () => {
            try {
                ...
                if (body) {
                    ...
                    const newEntries = Object.entries(newTpl);
                    const fieldsContainer = $('#tpl-fields');
                    if (fieldsContainer) {
                        fieldsContainer.innerHTML = newEntries.map(([key, v]) => renderField(key, v)).join('');
                        // 重新绑定字段事件
                        rebindFieldEvents();
                    }
                }
            } catch (e) { /* JSON 不合法，保留原内容 */ }
        });
    }

    // 标签行内编辑（双击变输入框，失焦或回车保存）
...
function applyHighlight(codeEl, text) {
    ...
    if (window.hljs) {
        try {
            codeEl.innerHTML = window.hljs.highlight(text, { language: 'json' }).value;
        } catch (e) { }
    }
}
...
function renderField(key, v) {
    ...
    if (Array.isArray(v)) {
        return `<div class="field-row">
            <label class="editable-label" data-key="${key}" title="双击编辑标签">${esc(labelText)}</label>
            <input class="input tmpl-field" data-field="${key}" value="${esc(JSON.stringify(v))}" placeholder="[]" />
            <button class="btn-icon btn-del-tmpl" data-del-key="${key}" title="删除字段">✕</button>
        </div>`;
    }
